...
for i = 1:height(data)
    ...
    start_time_in_seconds = seconds(timeofday(start_time));
...
for i = 1:length(sats)
    ...
    for j = 1:length(gs_iots)
        ...
        if ~isempty(relevant_data)
            ...
            first_start_time_seconds = seconds(timeofday(first_start_time));
...
for i = 1:length(delays)
    ...
    delay = delays{i}{3};
    sat = delays{i}{4};
    delay_str = sprintf('%ds (%s)', seconds(timeofday(delay)), sat);
...
p = plot(G_updated, 'Layout', 'force', 'NodeLabel', G_updated.Nodes.Name, ...
         'EdgeLabel', edg_labels, 'LineWidth', 1.5, 'MarkerSize', 8, 'EdgeFontSize', 14, ...
         'NodeFontSize',14);
...
highlight(p, "MainIoT", 'NodeColor', 'r', 'MarkerSize', 12);
highlight(p, "GS", 'NodeColor', 'g', 'MarkerSize', 12);
highlight(p, "Node 1", 'NodeColor', 'blue', 'MarkerSize', 10);
highlight(p, "Node 2", 'NodeColor', 'blue', 'MarkerSize', 10);
title("Node Graph representation of IoT Node Communication delays via Satellites")
grid on;
