    ...
    for numSats = 6:2:24  
        ...
        for mc_iter = 1:num_iterations
            ...
            if delay_fw == delay_bruteforce
                accurate_results = accurate_results +1;
            end
        end
        
        % Calculate averages
        accuracy = accurate_results / num_iterations;
    
        [~, cols1] = size(nodes_array);
        [~, cols2] = size(sats_array);

        % Store results
        newRow = table(cols1, cols2, accuracy, ...
                       'VariableNames', {'Number_of_Nodes', 'Number_of_Sats', 'FW Accuracy'});
        results = [results; newRow];
    end
end

% Write the results table to a CSV file
writetable(results, 'accuracy_results2.csv');
disp('Simulation complete. Results written to accuracy_results2.csv');
